...
workflow() %>% 
  add_recipe(train_rec) %>% 
  add_model(rf_model_tune) -> rf_wf_tune
rf_wf_tune
```
...
workflow() %>% 
  ...
  add_model(rf_model_tune) %>% 
  parameters() %>% 
  update(mtry = mtry(c(1, 143))) -> finalize_param # 手工update
finalize_param %>% pull_dials_object("mtry")

# 根据finalize得到的范围自动选择，finalize的结果要正确
finalize_param %>% grid_regular(levels = 3)
finalize_param %>% grid_regular(levels = c(min_n = 2, mtry = 5))
...
```{r}
# 查看结果：mtry范围正确
rf_wf_tune_res %>% collect_metrics() %>% arrange(mtry)
